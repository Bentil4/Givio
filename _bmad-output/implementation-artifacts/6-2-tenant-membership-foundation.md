...
### Second Review Pass (GitHub Copilot automated review, PR #79, 2026-09-25)

After the first review pass was pushed, GitHub's Copilot code review left 5 inline findings (2 high, 3 medium) on PR #79. 4 were valid and fixed; 1 was valid but deferred (already unreachable, matching this story's own documented scope boundary).

- [x] [Review][Patch] `createMembership`'s duplicate-active-Membership check is a non-atomic check-then-create — a concurrent request can pass the same pre-check before either write lands [functions/set-role-and-permissions/src/tenant-membership.js:175-177] — fixed: added a `unique` index on `memberships.userId` (verified live: a second concurrent-style insert for an existing `userId` gets rejected with `409 row_unique_constraint_violation`, even for a `revoked`-status second row), and `createRow`'s conflict error now translates to a clean `409` (`isConflictError`, promoted from `admin-users.js` to `shared.js` so both modules share one definition).
- [x] [Review][Patch] A failed sweep after a successful `setTenantStatus` write had no retry path — `ALLOWED_TENANT_TRANSITIONS` has no outgoing transitions for `suspended`/`rejected`, so a retry of the same call was rejected as an illegal no-op transition, leaving the tenant permanently stuck with an unswept sweep [functions/set-role-and-permissions/src/tenant-membership.js:328] — fixed: a same-status call to `suspended`/`rejected` now retries just the sweep (skips the already-done tenant-status write) instead of being rejected.
- [x] [Review][Patch] `listAllRows`'s pagination path (the `cursorAfter` branch) was added but never exercised by any test — every fixture returned fewer than 100 rows [functions/set-role-and-permissions/src/shared.js, functions/set-role-and-permissions/tests/tenant-membership.test.js] — fixed: new test drives a full 100-row first page + a second page, asserting both the `cursorAfter` query and that every row across both pages was processed.
- [x] [Review][Defer] A failed/unresolved `getMyActiveMembership()` lookup is indistinguishable from "confirmed no Membership" — an offline Membership-bearing caller's Event would sync unscoped rather than re-resolving its tenant later [src/app/data/services/event-data.service.ts:117-120] — deferred to `deferred-work.md`: currently unreachable (no Organizer-tier "Create Event" UI exists in Epics 6-9 yet), and the real fix needs a small redesign (tri-state resolution, re-resolve at sync time) best done by whichever story actually builds that flow.
