...
describe('app.routes', () => {
  ...
  // guards on a child-only navigation (e.g. /admin -> /admin/settings) — only
  // canActivateChild re-checks on every child activation. Without this, idle expiry would
  // only ever be enforced once per visit to the admin/organizer subtree.
  for (const path of ['admin', 'organizer']) {
