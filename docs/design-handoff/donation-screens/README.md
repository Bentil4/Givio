...
| File | Purpose |
|---|---|
| `app/data/models/donation.ts` | `Donation`, `DonationType`, `SyncStatus`, `DonationDraft`, `ConflictPair`. |
| `app/data/models/event.ts` | `DonationEvent`, `EventStatus`, `EventOperator`. |
| `app/feature/components/connection-banner/` | Offline / syncing / back-online banner. Pure presentational. |
| `app/feature/components/donation-form/` | The 6-field entry form. Emits a validated `DonationDraft`. |
| `app/feature/components/pending-queue/` | Queued-offline list with per-record edit / discard. |
| `app/feature/components/conflict-resolver/` | Two-version side-by-side comparison + three resolutions. |
| `app/feature/components/donation-row/` | One row of the donor list (family + operator views). |
| `app/feature/pages/organizer/donation-entry/` | Operator entry page: form + recent sidebar + confirm dialog. |
| `app/feature/pages/family/family-live/` | Family live view: total, breakdown, donor list. |
| `app/routes.additions.ts` | The route entries to merge into `app.routes.ts`. |
